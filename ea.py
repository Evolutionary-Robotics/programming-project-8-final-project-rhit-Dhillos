...
class MGA():
    ...
    def __init__(self, fitnessfunction, sound_grid, genesize, popsize, recomprob, mutationprob, tournaments, pop, x, y):
    ...
    def calculateFitness(self):
        ...
        for i in range(self.popsize):
            fits[i] = self.fitnessfunction(self.pop[i], self.sound_grid, self.x, self.y)
        return fits
    ...
    def run(self):
        ...
        for t in range(self.tournaments):
            ...
            # Update
            self.fit[loser] = self.fitnessfunction(self.pop[loser], self.sound_grid, self.x, self.y)
            # 6 Stats 
            if t % self.popsize == 0:
